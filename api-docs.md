...
- `GET /api/admin/dashboard/stores/:id/summary`
...
Ambil summary dashboard untuk satu store.

Permission:

- `dashboard:read` atau `store:read`
...
- `superAdmin` bisa melihat summary store mana pun.
- `storeAdmin` hanya bisa melihat summary store yang terhubung ke akunnya.
...
- `storeAdmins` hanya berisi admin aktif dengan role `storeAdmin` pada store tersebut.
- Data `storeAdmins` diurutkan berdasarkan `name` ascending.
- Cocok untuk halaman dashboard store atau panel detail store.
...
- `roleId` dan `storeId` memakai exact match UUID
- hanya user dengan role yang punya permission `admin:login` yang akan masuk list

Example query:

```txt
/api/admin/admin-accounts?q=admin&roleId=uuid&storeId=uuid&isVerified=true&sortBy=createdAt&sortOrder=desc&page=1&limit=10
```
...
Endpoint di module ini public. Response role hanya menyertakan field yang dibutuhkan frontend: `id`, `name`, `createdAt`, dan `updatedAt`.

### GET `/`

Ambil semua role aktif. Tidak memakai pagination karena jumlah role kecil.
...
- Data diurutkan berdasarkan `name` ascending.
- Role yang sudah soft deleted tidak ikut ditampilkan.

### GET `/:id`

Ambil detail role aktif beserta permission list.
...
- Permission list hanya menyertakan permission aktif.
- Permission diurutkan berdasarkan `name` ascending.
...
Ambil pilihan store aktif untuk dropdown/select. Endpoint ini public dan hanya mengembalikan field minimal.
...
- Data diurutkan berdasarkan `name` ascending.
- Store yang sudah soft deleted tidak ikut ditampilkan.
- Cocok untuk pilihan `storeId` di create/edit/filter admin account.
...
- Untuk form create/edit admin account dan filter admin account, frontend sebaiknya ambil pilihan dari `GET /api/roles` dan `GET /api/stores/options`, lalu kirim `roleId` dan `storeId`. Jangan minta user mengetik nama role/store manual.
