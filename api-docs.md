...
- Promo date-only strings are supported. For discount/voucher/report endpoints, `startDate=yyyy-MM-dd` becomes start of day UTC and `endDate=yyyy-MM-dd` becomes end of day UTC.
...
Get inventory-based product cards for a store, including stock `0` by default. Data is sourced from store stock rows, but response items use the same product-card shape as catalog pages with `storeStock`, `activeDiscount`, and `pricePreview`.

#### Frontend Usage

Use this as the canonical public catalog endpoint when the user has selected a store. It returns product cards with selected-store inventory in `storeStock`, so frontend can render availability, crossed-out price, discount badge, and promo label directly. Use `inStock=true` only when the UI should hide out-of-stock cards.
...
query = {
  ...
  maxPrice?: nonNegativeInt;
  inStock?: boolean;
  sortBy?: "stock" | "productName" | "sku" | "brand" | "price" | "categoryName" | "createdAt" | "updatedAt";
  sortOrder?: "asc" | "desc";
  page?: positiveInt;
  limit?: positiveInt; // capped at 100
}
...
| `sortBy`       | enum    | `productName` | `stock`, `productName`, `sku`, `brand`, `price`, `categoryName`, `createdAt`, `updatedAt`.           |
| `sortOrder`    | enum    | `asc`         | `asc` or `desc`.                                                                                     |
| `page`         | number  | `1`           | Positive integer.                                                                                    |
| `limit`        | number  | `10`          | Positive integer, capped at `100`.                                                                   |
...
GET /api/stocks/store/<store-id>?categoryId=<uuid>&minPrice=10000&maxPrice=50000&sortBy=price&sortOrder=asc
```
  ...
  "data": [
    {
      ...
      "activeDiscount": {
        "name": "Instant Food Payday Promo",
        "type": "percentage",
        "value": 10,
        "buyQuantity": null,
        "getQuantity": null,
...
`activeDiscount` is `null` when no active discount exists, when the discount is expired/future, or when its quota is exhausted. `pricePreview` is always present. For `buyXGetY`, `pricePreview.finalPrice` and `pricePreview.discountAmount` are `null` with `calculationMode: "quantityBased"` because the discount depends on selected quantity.

### GET `/stocks/store/:storeId/product/:slug`

Get one inventory-based product card for one product in one store. Returns `storeStock.stock: 0` when the stock row exists with zero quantity.

#### Frontend Usage

Use this endpoint when the product detail page only needs availability and discount display for the currently selected store. It returns the same product-card shape as the stock catalog endpoint, without array/meta wrapper.
...
Use this for product-first store-scoped pages. Only products with an active stock row in the selected store are returned. Send `inStock=true` when the catalog should only show products available in the selected store. If `inStock` is omitted, available products are returned first and out-of-stock products are placed at the end. The main public catalog should use `GET /api/stocks/store/:storeId`, which returns the same product-card shape plus discount display fields.
...
Use this for product-first detail pages after the user has selected a store. The main public product detail should use `GET /api/stocks/store/:storeId/product/:slug` when it needs discount display fields.
...
Product discounts are store-scoped promos. They are managed by admins now and are intended to be auto-applied by the backend checkout flow later. Frontend must not send `discountId` for this new product discount flow.

### Discount Rules

- Requires admin auth and the matching `discount:*` permission.
- `superAdmin` can manage discounts for any active store.
- `storeAdmin` can manage discounts only in their assigned store; body/query `storeId` is ignored for scoping.
- Each discount is bound to one `productId + storeId`.
- Product and store must be active, and the product must have an active stock row in the target store.
- Only one active discount may exist for the same `productId + storeId` in overlapping date windows.
- `percentage` requires `value` from `1` to `100`.
- `nominal` requires positive `value`.
- `buyXGetY` requires positive `buyQuantity` and `getQuantity`.
- `quota` is optional. Omitted means unlimited discount usage.
- `usedQuota` starts at `0` and is intended to be incremented by the future checkout flow.
- `startDate` must be before or equal to `endDate`.

### GET `/admin/discounts`

Get paginated admin discount list.
...
Get one discount detail. StoreAdmins receive `404` for discounts outside their assigned store.
...
body = {
  name: string;
  type: "percentage" | "nominal" | "buyXGetY";
  value?: positiveInt; // required for percentage/nominal
  buyQuantity?: positiveInt; // required for buyXGetY
  getQuantity?: positiveInt; // required for buyXGetY
...
Update a discount. Same body fields as create, all optional, at least one required. `usedQuota` is not admin-writable.
...
Vouchers are selected explicitly by frontend and sent to checkout as `voucherId` and/or `deliveryVoucherId`. Vouchers can be global (`storeId: null`) or store-scoped.

### Voucher Rules

- Requires admin auth and the matching `voucher:*` permission.
- `superAdmin` can create global vouchers with `storeId: null`, or store vouchers with a concrete `storeId`.
- `storeAdmin` can manage only vouchers assigned to their store; body/query `storeId` is ignored for scoping.
- `code` is normalized to uppercase and must be globally unique.
...
Get paginated admin voucher list.
...
Get one voucher detail. StoreAdmins receive `404` for global vouchers or other-store vouchers.
...
Update a voucher. Same body fields as create, all optional, at least one required.
...
Promo reports read immutable history snapshot fields, not current master data. This keeps reports stable even after discount/voucher names or values are changed.

### GET `/admin/promo-reports/discounts`

Read discount usage from `DiscountHistory`.
...
Read voucher usage from `VoucherHistory`.
